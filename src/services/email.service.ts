...
export const sendBookingNotificationToAdmin = async (
  ...
  const guestInfoHtml = isGuestBooking 
    ? `
      <p style="margin: 8px 0; font-size: 16px;"><strong>📧 Guest Email:</strong> ${(bookingData as GuestBookingData).guestEmail}</p>
      <p style="margin: 8px 0; font-size: 16px;"><strong>📱 Guest Phone:</strong> ${(bookingData as GuestBookingData).guestPhone}</p>
    ` 
    : '';
