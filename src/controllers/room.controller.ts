  ...
  try {
    ...
    if (!room.is_bookable) {
      if (room.static_status === 'maintenance') {
        ...
        const { rows: nextAvailableRows } = await pool.query(
...
export const getHomestayRoomsStatus = async (
  ...
  try {
    ...
    const { rows: homestayRows } = await pool.query(
      'SELECT id FROM "homestay" WHERE id = $1',
      [homestayIdNum]
    );
...
export const getRoomBlockedPeriods = async (
  req: Request,
  res: Response,
  next: NextFunction
) => {
  try {
    ...
    if (start_date && !dateRegex.test(start_date as string)) {
      return next(new AppError('Invalid start_date format. Use YYYY-MM-DD', 400));
    }
    if (end_date && !dateRegex.test(end_date as string)) {
      return next(new AppError('Invalid end_date format. Use YYYY-MM-DD', 400));
    }

    // Default date range: today to 3 months ahead
    const today = new Date();
    const startDate = (start_date as string) || today.toISOString().split('T')[0];
    const futureDate = new Date(today.getTime() + 90 * 24 * 60 * 60 * 1000);
    const endDate = (end_date as string) || futureDate.toISOString().split('T')[0];

    // Check if room exists
    const { rows: roomRows } = await pool.query(
      'SELECT id, title, room_number FROM "homestayRoom" WHERE id = $1',
      [roomIdNum]
    );
