...
import { isTestMode } from '../config/xendit.config';
...
export const createQRISPayment = async (
  ...
  try {
    ...
    const qrisPayment = await qrisService.createQRISPayment({
      booking_id: booking_id.toString(),
      amount: booking.total_price,
      customer_name,
      customer_email,
...
export const checkQRISPaymentStatus = async (
  ...
  try {
    ...
    if (payment.transaction_id && payment.payment_status === 'PENDING') {
      try {
        const xenditStatus = await qrisService.checkPaymentStatus(payment.transaction_id);
 ...
 */
export const handleQRISWebhook = async (
  req: Request,
  res: Response,
  next: NextFunction
) => {
  try {
    ...
    // Process the webhook
    await qrisService.processWebhook(webhookData);
...
export const confirmManualPayment = async (
  ...
  try {
    ...
    if (owner_confirmation !== true) {
      return next(new AppError('Owner confirmation is required', 400));
    }
    ...
    await pool.query(
      'UPDATE "booking" SET status = $1, confirmed_at = CURRENT_TIMESTAMP WHERE id = $2',
      ['confirmed', booking_id]
    );

    // Update payment to indicate manual confirmation
    await pool.query(
      'UPDATE payments SET manual_confirmation_required = $1 WHERE booking_id = $2',
      [false, booking_id]
    );
...
export const simulatePaymentSuccess = async (
  ...
  try {
    if (!isTestMode()) {
      return next(new AppError('Payment simulation only available in test mode', 403));
    }

    const { booking_id } = req.params;

    await qrisService.simulatePaymentSuccess(booking_id);
  ...
  try {
    ...
    res.json({
      ...
      xendit_config: {
        ...
        secret_key_type: xenditConfig.secretKey.includes('development') ? 'test' : 'production'
      }
    });
