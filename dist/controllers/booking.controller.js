    ...
    try {
        ...
        try {
            ...
            if (isBookingForToday) {
                ...
                const { rows: activeBookings } = await client.query(`SELECT * FROM "booking" 
           WHERE room_id = $1 
             AND status IN ($2, $3)
             AND start_date <= $4 
             AND end_date >= $4`, [room_id, booking_types_1.BookingStatus.CONFIRMED, booking_types_1.BookingStatus.PENDING, start_date]);
                ...
                const { rows: completedToday } = await client.query(`SELECT * FROM "booking" 
           WHERE room_id = $1 
             AND status = $2
             AND start_date <= $3 
             AND end_date >= $3
             AND updated_at::date = $3::date
           ORDER BY updated_at DESC`, [room_id, booking_types_1.BookingStatus.COMPLETED, start_date]);
                if (completedToday.length > 0) {
                    ...
                    if (now >= housekeepingCompleteTime) {
                        ...
                        return next(new error_middleware_1.AppError(`Room will be available after housekeeping completion in ${minutesLeft} minutes`, 400));
                    }
                }
                else {
                ...
                const requestStart = (0, moment_1.default)(start_date).startOf('day');
                const requestEnd = (0, moment_1.default)(end_date).startOf('day');
           ...
           AND status IN ($2, $3)`, [room_id, booking_types_1.BookingStatus.CONFIRMED, booking_types_1.BookingStatus.PENDING]);
                ...
                for (const booking of allBookings) {
                    const bookingStart = (0, moment_1.default)(booking.start_date).startOf('day');
                    const bookingEnd = (0, moment_1.default)(booking.end_date).startOf('day');
...
const updateBookingStatus = async (req, res, next) => {
    ...
    try {
       ...
       RETURNING *`, [
        ...
        if (status === booking_types_1.BookingStatus.COMPLETED && oldStatus !== booking_types_1.BookingStatus.COMPLETED) {
            // Booking completed - room becomes available (early checkout scenario)
            await client.query(`UPDATE "homestayRoom" 
         SET status = 'available', updated_at = CURRENT_TIMESTAMP 
         WHERE id = $1`, [booking.room_id]);
            console.log(`[BOOKING] Room ${booking.room_id} marked as available after booking ${id} completion`);
        }
        else if (status === booking_types_1.BookingStatus.CANCELLED && oldStatus !== booking_types_1.BookingStatus.CANCELLED) {
        ...
        else if (status === booking_types_1.BookingStatus.CONFIRMED && oldStatus !== booking_types_1.BookingStatus.CONFIRMED) {
...
const checkRoomAvailability = async (req, res, next) => {
    try {
        ...
        const { rows: allBookings } = await database_1.pool.query(`SELECT id, start_date, end_date, status, booking_number 
       FROM "booking" 
       WHERE room_id = $1 
       AND status IN ('confirmed', 'pending')
       ORDER BY start_date ASC`, [roomIdNum]);
        // Check for conflicts with the requested dates
        const { rows: conflictingBookings } = await database_1.pool.query(`SELECT id, start_date, end_date, status 
       FROM "booking" 
       WHERE room_id = $1 
       AND status IN ('confirmed', 'pending')
       AND NOT (end_date <= $2 OR start_date >= $3)`, [roomIdNum, start_date, end_date]);
        console.log(`[SIMPLE-CHECK] Found ${conflictingBookings.length} conflicting bookings`);
        const isAvailable = conflictingBookings.length === 0;
        // Auto-fix room status
        if (isAvailable) {
            await database_1.pool.query('UPDATE "homestayRoom" SET status = $1 WHERE id = $2', ['available', roomIdNum]);
        }
        ...
        const formatBookingForResponse = (booking) => {
            if (!booking)
                return null;
            return Object.assign(Object.assign({}, booking), { start_date: formatDate(booking.start_date), end_date: formatDate(booking.end_date) });
        };
...
const getRoomBookings = async (req, res, next) => {
    try {
        ...
        const { rows: roomRows } = await database_1.pool.query('SELECT id FROM "homestayRoom" WHERE id = $1', [roomIdNum]);
        if (roomRows.length === 0) {
            return next(new error_middleware_1.AppError('Room not found', 404));
        }
...
const checkSameDayAvailability = async (req, res, next) => {
    try {
        const { roomId } = req.params;
        ...
        const { rows: bookings } = await database_1.pool.query(`SELECT b.*, 
              EXTRACT(EPOCH FROM (b.updated_at AT TIME ZONE 'UTC')) as updated_timestamp
       FROM "booking" b
       WHERE b.room_id = $1 
         AND b.start_date <= $2 
         AND b.end_date >= $2
       ORDER BY b.updated_at DESC`, [roomIdNum, checkDate]);
        ...
        const completedToday = bookings.filter(booking => {
            const isCompleted = booking.status === booking_types_1.BookingStatus.COMPLETED;
            const updatedToday = new Date(booking.updated_at).toISOString().split('T')[0] === checkDate;
            return isCompleted && updatedToday;
        });
        const activeBookings = bookings.filter(booking => [booking_types_1.BookingStatus.CONFIRMED, booking_types_1.BookingStatus.PENDING].includes(booking.status));
...
exports.checkSameDayAvailability = checkSameDayAvailability;
