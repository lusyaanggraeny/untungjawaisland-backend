...
function parseRooms(roomString) {
    if (!roomString)
        return { numRooms: null, isWholeHome: false };
    let numRooms = null;
...
async function ensureDefaultAdminExists(client) {
    ...
    if (checkAdmin.rows.length === 0) {
        ...
        const status = 'active';
        await client.query(`INSERT INTO admin_users (id, username, email, password_hash, full_name, role, status, created_at, updated_at, last_login)
       VALUES ($1, $2, $3, $4, $5, $6, $7, NOW(), NOW(), NULL) ON CONFLICT (id) DO NOTHING`, [DEFAULT_ADMIN_ID, username, email, hashedPassword, fullName, role, status]);
...
async function ensureOwnerUsers(client, ownerNames, superAdminId) {
    const ownerMap = {};
    for (const ownerName of ownerNames) {
        ...
        const insert = await client.query(`INSERT INTO admin_users (username, password, email, name, role, is_active, created_at, updated_at)
       VALUES ($1, $2, $3, $4, $5, $6, NOW(), NOW()) RETURNING id`, [username, hashedPassword, email, ownerName, role, is_active]);
...
async function seedFromCSV() {
    ...
    try {
        ...
        const csvFilePath = path_1.default.join(__dirname, '../../../IMAGES OF UNTUNG JAWA ISLAND - Sheet1.csv');
        ...
        for (const row of parsedRows) {
            ...
            try {
                const homestayRes = await client.query(`INSERT INTO homestay (title, description, user_id, status, has_rooms, location, address, base_price, max_guests, contact_number, category, created_at, updated_at)
           ...
           RETURNING id`, [
                ...
                await client.query(`INSERT INTO "homestayImages" (homestay_id, img_url, is_primary, "order", created_at, updated_at)
           VALUES ($1, $2, $3, $4, NOW(), NOW())`, [homestayId, imageInput.img_url, imageInput.is_primary, imageInput.order]);
                SUCESSFUL_INSERTS++;
